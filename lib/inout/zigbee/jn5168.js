...
// Zigbee GPIO request for local or remote end point.
function api_adc_req(unicast_add, pin) {
	...
	{ 
		var API_adc_req = API_remote_REQ.fields;
		var buf_adc_req = API_remote_REQ.buffer();
		APIframe.idAPI = API_ID.REMOTE_AT_REQ;
		API_adc_req.frame_id = 0xEC;
		API_adc_req.option = 0;
	}

	// Set common parameters.
	API_adc_req.AT_index = API_INDEX.ATAD;
	API_adc_req.param_value[0] = pin;
	API_adc_req.param_value[1] = pin;
	API_adc_req.param_value[2] = pin;// Dummy.
	API_adc_req.param_value[3] = pin;// Dummy.	
	API_adc_req.param_value[4] = unicast_add[0];
	API_adc_req.param_value[5] = unicast_add[1];
	APIframe.len = buf_adc_req.length-1;	

	// Calculate Checksum
	API_adc_req.checksum  = calcChecksum(buf_adc_req, 0, (buf_adc_req.length-1));

	// Build the API message.
	var ATmsg = new Buffer(bufAPIframe.length + buf_adc_req.length);
	for(i=0;i<bufAPIframe.length;++i)
		ATmsg[i] = bufAPIframe[i];
	for(i=0;i<buf_adc_req.length;++i)
		ATmsg[bufAPIframe.length + i] = buf_adc_req[i];
...
function api_process(response) {
	...
	if(checkSum == response[(response.length-1)]) {
		...
		// ADC Response
		if(response[2] == API_INDEX.ATAD){
			ADC.uni_add = response[5];
			ADC.uni_add |= response[4]<<8;
			ADC.pin = response[15];
			ADC.val = response[17];
			ADC.val |= response[16]<<8;
			console.log(JSON.stringify(ADC));
		}
...
function api_send(data, callback) {
	...
	api_adc_req(uni_add, analog_pins.TEMP);
}
